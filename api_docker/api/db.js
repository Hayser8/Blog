...
export async function createBlog(title, content, banner) {
    try {
        ...
        const [newBlog] = await conn.query("SELECT * FROM blog_posts WHERE id = ?", [insertedId]);
...
export async function updateBlog(id, title, content, banner) {
    try {
        await conn.query(`UPDATE blog_posts SET title='${title}', content='${content}', banner='${banner}' WHERE id=${id}`);
        const [updatedRows] = await conn.query(`SELECT * FROM blog_posts WHERE id = ${id}`);
